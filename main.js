(() => {
  ...
  let   G = 5.6;                     // default gravitational constant (mutable)
  ...
  let   SIZE_VAR_MIN = 0.31 / BASE_R_WORLD;     // default min radius = 0.31 units
  let   SIZE_VAR_MAX = 1.5  / BASE_R_WORLD;     // default max radius = 1.5 units
  ...
  let   SIZE_BIAS = 10.0;            // default bias k (>1 = smaller)
  ...
  // --- View state ---
  let scale = 3;      // starting zoom: 3 px/unit
  ...
  function sizeMulAt(x, y) {
    ...
    // Effective radius in world units = BASE_R_WORLD * sizeMul
  }
  ...
  function render() {
    ...
    if (active) {
      ...
      // Thrusters still operate when gravity is off
    ...
    // Highlight selected with blue stroke (original thickness)
    if (selectedId) {
      const b = bodies.get(selectedId);
      if (b) {
        const cx = (b.x - viewX) * scale;
  ...
  canvas.addEventListener('wheel', (e) => {
    ...
    scale *= factor;
    // Clamp between MIN_SCALE (max zoom-out) and MAX_SCALE (max zoom-in)
    scale = Math.min(Math.max(scale, MIN_SCALE), MAX_SCALE);

    // Keep mouse-world point fixed while zooming
